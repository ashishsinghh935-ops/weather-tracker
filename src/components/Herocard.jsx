...
const HeroCard = ({ weatherData, locationName }) => {
  ...
  return (
    // UPGRADED: Deep Glassmorphism Container with semi-transparent gradients and heavy blur
    <div className="w-full relative overflow-hidden rounded-3xl p-8 text-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/20 backdrop-blur-xl bg-gradient-to-br from-indigo-600/90 via-purple-600/80 to-fuchsia-500/90">
      
      {/* Abstract Glass Glow Effects inside the card */}
      <div className="absolute -top-24 -right-24 w-64 h-64 bg-white/20 rounded-full blur-3xl pointer-events-none"></div>
      <div className="absolute -bottom-24 -left-24 w-48 h-48 bg-indigo-300/20 rounded-full blur-2xl pointer-events-none"></div>
      ...
      <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div className="flex flex-col">
          ...
          {/* Enhanced Temperature Display with tracking and heavy drop shadow */}
          <div className="text-7xl font-extrabold mb-2 tracking-tighter drop-shadow-lg">
            {temp !== undefined && temp !== null ? Math.round(temp) : "--"}°C
          </div>
          
          <div className="text-xl font-medium text-indigo-50 mb-6 drop-shadow-md">
            {condition.text}
          </div>
          
          <div className="flex flex-wrap gap-4">
            {/* UPGRADED: Frosted Glass Badges with internal borders and shadow */}
            <div className="flex items-center bg-white/10 border border-white/20 rounded-xl px-4 py-2.5 backdrop-blur-md shadow-[0_4px_15px_rgb(0,0,0,0.1)]">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-indigo-200" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex items-center bg-white/10 border border-white/20 rounded-xl px-4 py-2.5 backdrop-blur-md shadow-[0_4px_15px_rgb(0,0,0,0.1)]">
              ...
              <span className="text-sm font-semibold text-white">Wind: {wind} km/h</span>
            </div>
          </div>
        </div>
        
        {/* Massive 3D pop effect for the icon on hover */}
        <div className="text-9xl drop-shadow-[0_15px_35px_rgba(0,0,0,0.3)] transform hover:scale-110 transition-transform duration-500 cursor-default">
          {condition.icon}
        </div>
      </div>
    </div>
