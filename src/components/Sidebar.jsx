...
const Sidebar = ({ setLocation, isMobileMenuOpen, setIsMobileMenuOpen }) => {
  ...
  const handleSelectCity = async (cityName) => {
    setSearchQuery('');
    setShowDropdown(false);
    if (setIsMobileMenuOpen) setIsMobileMenuOpen(false); // Close menu on mobile after selection
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        ...
        setLocation({ lat: city.latitude, lon: city.longitude, name: cityName });
        navigate('/');
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      {/* UPGRADED: Responsive Drawer Classes */}
      <div className={`fixed inset-y-0 left-0 transform ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-50 w-72 md:w-64 bg-white border-r border-gray-200 flex flex-col p-4 shadow-2xl md:shadow-sm`}>
        ...
        <div className="mb-8 relative" ref={dropdownRef}>
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all"
            />
          </div>

          {showDropdown && suggestions.length > 0 && (
            <ul className="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg z-50 overflow-hidden max-h-60 overflow-y-auto">
              {suggestions.map((cityName, index) => (
                <li 
                  key={index}
                  onClick={() => handleSelectCity(cityName)}
                  className="px-4 py-3 md:py-2 hover:bg-indigo-50 cursor-pointer text-sm border-b border-gray-100 last:border-b-0"
                >
        ...
        <div className="flex-1">
          ...
          <nav className="space-y-2">
            ...
            <Link 
              ...
              onClick={() => setIsMobileMenuOpen(false)}
              className={`flex items-center space-x-3 px-4 py-3 md:py-2.5 rounded-lg transition-colors duration-200 ${
                isActive('/map') ? 'bg-indigo-50 text-indigo-700 font-semibold' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900'
              }`}
            >
