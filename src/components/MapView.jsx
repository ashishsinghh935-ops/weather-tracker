      ...
      <Popup className="rounded-lg shadow-sm">
        {loading ? (
          ...
          <div className="flex flex-col space-y-3 min-w-[200px] p-1">
            ...
            <div className="text-2xl font-bold text-indigo-600 my-2">{weatherData.temp}°C</div>
            ...
            <button 
              ...
              onClick={() => {
                setLocation({ lat: parseFloat(weatherData.lat), lon: parseFloat(weatherData.lon), name: weatherData.name });
                navigate('/');
              }}
            >
...
const MapView = ({ location, setLocation, weatherData }) => {
  ...
  return (
    <div className="h-full w-full flex flex-col space-y-2">
      <div className="mb-2">
        {/* Dynamic Text Colors to match Dashboard */}
        <h2 className={`text-2xl font-bold ${isDay ? 'text-gray-900' : 'text-white'}`}>Interactive Radar & Point-and-Click Weather</h2>
        <p className={`text-sm mt-1 ${isDay ? 'text-gray-500' : 'text-zinc-400'}`}>Click anywhere on the map to instantly scan atmospheric data for that coordinate.</p>
      </div>
      
      <div className="flex-1 w-full rounded-xl overflow-hidden border border-gray-200 shadow-sm z-0 relative min-h-[600px]">
        {showRadar && <WindLayer windSpeed={actualWindSpeed} windDirection={actualWindDirection} weatherCode={actualWeatherCode} />}

        <button onClick={handleLocateMe} disabled={isLocating} className="absolute top-4 right-4 z-[400] bg-white p-3 rounded-full shadow-lg border border-gray-200 hover:bg-gray-50 transition-all group disabled:opacity-70 flex items-center justify-center cursor-pointer">
          {isLocating ? (
            <svg className="animate-spin h-6 w-6 text-indigo-600" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
          ) : (
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-gray-700 group-hover:text-indigo-600 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M12 2v2"></path><path d="M12 20v2"></path><path d="M4 12H2"></path><path d="M22 12h-2"></path><circle cx="12" cy="12" r="8"></circle></svg>
          )}
        </button>

        <button onClick={() => setShowRadar(!showRadar)} className={`absolute top-20 right-4 z-[400] p-3 rounded-full shadow-lg border transition-all group flex items-center justify-center cursor-pointer ${showRadar ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
          <svg xmlns="http://www.w3.org/2000/svg" className={`h-6 w-6 ${showRadar ? 'text-indigo-600' : 'text-gray-700 group-hover:text-indigo-600 transition-colors'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M9.59 4.59A2 2 0 1 1 11 8H2m10.59 11.41A2 2 0 1 0 14 16H2m15.73-8.27A2.5 2.5 0 1 1 19.5 12H2" /></svg>
        </button>

        <MapContainer center={[location.lat, location.lon]} zoom={10} style={{ height: '100%', width: '100%', zIndex: 0 }}>
          {/* REVERTED TO PURE BRIGHT OSM TILES */}
          <TileLayer
          ...
          <Marker position={[location.lat, location.lon]}>
            <Popup className="font-semibold text-indigo-600">Active Location:<br/><span className="text-gray-800">{location.name}</span></Popup>
          </Marker>
